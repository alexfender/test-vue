		...
		Представьтесь:
		  <input id="username" type="text" class="form-control" 
			...
			<div class="row g-3" style="margin-top: 5px;">
				...
				<div class="col-auto">
					<button id="sock-send-butt" type="button" value="send" @click="sendMessage()" class="btn btn-primary mb-3">Отправить</button>
				</div>
			</div>
		...
		methods: {
			...
			sendMessage() {
				...
				if (!this.username) {
					...
					if (this.message) {
						let message = '{"username": "'+ this.username +'","userId": "'+ this.userId +'", "message":"'+ this.message +'"}';
			...
			onInput(e) {
				...
				if (!this.tiping) {
					this.tiping = true;
					this.socket.send('{"username": "'+ this.username +'","userId": "'+ this.userId +'", "action": "tiping", "tiping": true}');
				};
				
				clearTimeout(this.typingTimer);
				this.typingTimer = setTimeout(() => {
					this.tiping = false;
					this.socket.send('{"username": "'+ this.username +'","userId": "'+ this.userId +'", "action": "tiping", "tiping": false}');
				}, 1000);
